#!/usr/bin/env python3
"""
Pester - PreToolUse Hook Script
Intercepts Claude Code tool calls and forwards them to the Pester overlay app
...
def is_claude_allowed(tool_name, tool_input):
    ...
    If so, Pester should not intercept — let Claude Code handle it silently."""
    settings_paths = [
...
def check_server():
    """Check if Pester app is running."""
    try:
...
def send_cost_update(session_id, cost_usd):
    """Fire-and-forget cost update to Pester."""
    if cost_usd <= 0:
        return
    try:
        payload = json.dumps({
...
def send_permission_request(tool_name, tool_input, session_id):
    """Send a permission request to Pester and wait for response."""
    payload = json.dumps({
        "tool_name": tool_name,
...
def poll_for_decision(request_id, timeout=60):
    """Poll Pester for the user's decision. Blocks until response or timeout."""
...
def is_bypass_mode(config):
    ...
    Also returns True if notify_only is set in pester config."""
...
def main():
    ...
    if not claude_allowed:
        ...
        # handle the Pester prompt. PreToolUse only enforces always_block here to
        # prevent a double-prompt (PreToolUse + PermissionRequest both showing Pester UI).
    ...
    # so PreToolUse is the only Pester gatekeeper here.
    ...
    if decision == "allow":
        result = {
            "hookSpecificOutput": {
                "hookEventName": "PreToolUse",
                "permissionDecision": "allow",
                "permissionDecisionReason": "Approved via Pester"
            }
        }
        print(json.dumps(result))
        sys.exit(0)
        ...
        reason = message if message else "Denied via Pester"
        result = {
            "hookSpecificOutput": {
                "hookEventName": "PreToolUse",
                "permissionDecision": "deny",
